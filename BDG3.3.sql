...
DROP TRIGGER PosicaoPK; /*DONE*/
DROP TRIGGER EquipaPK; /*DONE*/
DROP TRIGGER EstadioPK; /*DONE*/
DROP TRIGGER check_nif_before_insert_new_manager; /*DONE*/
DROP TRIGGER check_nif_before_insert_new_player; /*DONE*/
DROP TRIGGER check_if_one_red_before_insert_new_summon; /*DONE*/
DROP TRIGGER check_if_player_is_expelled_before_insert_action; /*DONE*/
...
--Não funciona
/*DROP TRIGGER check_if_two_yellow_before_insert_new_summon;
CREATE OR REPLACE TRIGGER check_if_two_yellow_before_insert_new_summon
    BEFORE INSERT OR UPDATE ON ACOES
    FOR EACH ROW
    DECLARE
    ...
    BEGIN
        IF :NEW.TipoAcaoID = 7 THEN
            -- Count the occurrences of TipoAcaoID = 8 for the given NIF, DataJogo, and Hora
            SELECT COUNT(*)
            INTO ContagemAmarelos
            FROM Acoes
            WHERE NIF = :NEW.NIF AND DataJogo = :NEW.DataJogo AND Hora = :NEW.Hora
              AND TipoAcaoID = 7;
...
INSERT INTO Acoes(Hora, DataJogo, EquipaID, NIF, Minuto, TipoAcaoID)
VALUES(2100, '2023-07-04', 1, 204805103, 70, 2);
commit;

select * from convocatoria where hora = 2100 and nif = 204805103;
select * from acoes where hora = 2100 and nif = 204805103;
*/
