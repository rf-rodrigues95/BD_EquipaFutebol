...
CREATE TABLE Pessoas (
    ...
    CONSTRAINT morada_pessoas_fk FOREIGN KEY (lOCALIDADE) REFERENCES MORADA (LOCALIDADE) ON DELETE CASCADE
);

CREATE TABLE Treinadores (
	NIF NUMBER NOT NULL,
	Cargo varchar(250) NOT NULL,
	CONSTRAINT fk_Treinadores_Pessoa FOREIGN KEY (NIF) REFERENCES Pessoas (NIF) ON DELETE CASCADE,	
	CONSTRAINT treinadores_pk PRIMARY KEY (NIF)
);

CREATE TABLE Jogador (
	NIF NUMBER NOT NULL,
	CONSTRAINT fk_jogadores_Pessoa FOREIGN KEY (NIF) REFERENCES Pessoas (NIF) ON DELETE CASCADE,	
...
CREATE TABLE Joga_Posicao (
	...
	CONSTRAINT fk_jogadores_joga FOREIGN KEY (NIF) REFERENCES Jogador (NIF) ON DELETE CASCADE,		
	CONSTRAINT fk_posicao_joga FOREIGN KEY (PosicaoID) REFERENCES Posicao (PosicaoID) ON DELETE CASCADE,	
...
CREATE TABLE Estadio (
    ...
    CONSTRAINT fk_estadio_localidade FOREIGN KEY (lOCALIDADE) REFERENCES MORADA (LOCALIDADE) ON DELETE CASCADE
);
...
CREATE TABLE Equipas (
	...
	CONSTRAINT fk_EstadioID FOREIGN KEY (EstadioID) REFERENCES Estadio (EstadioID)	ON DELETE CASCADE
);
...
CREATE TABLE Jogo (
	...
	ResultadoC NUMBER NOT NULL,
    ResultadoF NUMBER NOT NULL,
	...
	CONSTRAINT fk_equipa_jogo FOREIGN KEY (EquipaID) REFERENCES Equipas (EquipaID) ON DELETE CASCADE,	
...
CREATE TABLE Convocatoria (
	...
	CONSTRAINT fk_convocatoria_jogo FOREIGN KEY (Hora,DataJogo,EquipaID) REFERENCES Jogo (Hora,DataJogo,EquipaID) ON DELETE CASCADE,	

	CONSTRAINT fk_convocatoria_jogador FOREIGN KEY (NIF) REFERENCES Jogador (NIF) ON DELETE CASCADE,	
...
CREATE TABLE TipoAcao (
 ...
 CREATE TABLE Acoes (
	...
	CONSTRAINT fk_Acoes_convocatoria FOREIGN KEY (NIF,Hora,DataJogo,EquipaID) REFERENCES Convocatoria (NIF,Hora,DataJogo,EquipaID) ON DELETE CASCADE,	

	CONSTRAINT fk_acoes_tipo FOREIGN KEY (TipoAcaoID) REFERENCES TipoAcao (TipoAcaoID) ON DELETE CASCADE,	
...
CREATE TABLE Transferencias (
	...
	CONSTRAINT fk_transferencias_tipo FOREIGN KEY (TipoTID) REFERENCES TipoT (TipoTID) ON DELETE CASCADE,	
    CONSTRAINT fk_transferencia_nif FOREIGN KEY (NIF) REFERENCES Jogador (NIF) ON DELETE CASCADE,
...
CREATE TABLE Transferencia_equipa(
	...
	CONSTRAINT fk_transferencia_te FOREIGN KEY (TransferenciaID) REFERENCES Transferencias (TransferenciaID) ON DELETE CASCADE,	

	CONSTRAINT fk_equipa_te FOREIGN KEY (EquipaID) REFERENCES Equipas (EquipaID) ON DELETE CASCADE,	
...
CREATE OR REPLACE TRIGGER check_resultado
BEFORE INSERT OR UPDATE ON Jogo
FOR EACH ROW
DECLARE
BEGIN
    IF :NEW.Casa = 1 AND :NEW.ResultadoC < :NEW.ResultadoF THEN
        :NEW.Valor := 'D';
    END IF;
    ...
    IF :NEW.Casa = 0 AND :NEW.ResultadoC < :NEW.ResultadoF THEN
        :NEW.Valor := 'V';
    END IF;
    ...
    IF :NEW.ResultadoC = :NEW.ResultadoF THEN
        :NEW.Valor := 'E';
    END IF;    
END;
/
...
CREATE OR REPLACE FORCE VIEW "JOGOSVIEW" ("DATAJOGO", "CASA", "EQUIPAID", "HORA", "RESULTADOC","RESULTADOF", "VALOR") AS 
  SELECT 
  J.DATAJOGO, J.CASA, J.EQUIPAID, J.HORA, J.RESULTADOC,J.RESULTADOF, J.VALOR
FROM JOGO J
/
...
/* JOGO */
INSERT INTO JOGO (Hora, DataJogo,ResultadoC,ResultadoF,Casa ,EquipaID)
...
INSERT INTO JOGO (Hora, DataJogo,ResultadoC,ResultadoF,Casa ,EquipaID)
VALUES (1900, DATE '2023-07-05', 3,3, 0, 1);
COMMIT;
/* CONVOCATORIA */
-- JOGO 1 -- 
...
SELECT RESULTADOC,RESULTADOF,CASA,VALOR FROM JOGO;
